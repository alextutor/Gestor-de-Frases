import keyboard  # Librería de bajo nivel
import pyperclip # Para manejo limpio del portapapeles
...
class PromptVaultApp(ctk.CTk):
    ...
    def auto_escribir(self, texto):
        """
        Técnica mejorada: Minimiza, asegura foco con clic y escribe.
        """
        # Guardar posición antes de minimizar
        x, y = pyautogui.position()
        
        # Minimizar la ventana actual
        self.iconify()
        
        # Pausa para que el i5-4440 gestione el cambio de ventana
        time.sleep(1.5)

        try:
            # Asegurar foco haciendo clic donde estaba el mouse
            pyautogui.click(x, y)
            time.sleep(0.5)
            
            # Escribir el prompt carácter por carácter

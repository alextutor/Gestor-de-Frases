...
def cargar_configuracion_global():
    global TEMA_ACTUAL, CONFIG_MODO_DETECCION, CONFIG_DIAS_PURGA
    DB_NAME = "prompts.sqlite"
    try:
...
def ejecutar_purga_automatica():
    ...
    try:
        conn = sqlite3.connect(DB_NAME)
        cursor = conn.cursor()
        # Elimina registros eliminados lógicamente cuyo lapso de días haya expirado nativamente
...
def iniciar_aplicacion():
    ...
    ejecutar_purga_automatica() # <--- Purga limpia tras iniciar y cargar configs

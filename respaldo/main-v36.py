...
DESCRIPCIONES = {
    ...
    "automatico": (
        ...
        "Si detecta aplicaciones de mensjaria (WhatsApp, Slack, Telegram),\n"
...
class VentanaCrudCategorias(ctk.CTkToplevel):
    def __init__(self, master, *args, **kwargs):
        super().__init__(master, *args, **kwargs)
        self.title("Administrar Categorías v36")
        self.transient(master)
        self.grab_set()
...
class VentanaCrudTemas(ctk.CTkToplevel):
    def __init__(self, callback_recargar_menu, *args, **kwargs):
        ...
        self.title("Administrar Temas Avanzado v36")
...
# ==========================================
# INTERFAZ Y LOGICA DE NAVEGACIÓN v36 (FILTRO EXCLUSIVO)
# ==========================================
def gestionar_navegacion_teclado(event):
    """ Módulo centralizado con filtro estricto por Widget para evitar rebotes """
    global INDICE_SELECCION_TECLADO, id_prompt_actual
    cantidad_tarjetas = len(TARJETAS_VISUALES_CACHE)
    if cantidad_tarjetas == 0: 
        return "break" if event.widget == txt_buscar else None

    # INTERCEPCIÓN DE REBOTE (OPCION B):
    # Si el evento es disparado por 'root' (ventana global) pero tu cursor real está
    # situado dentro del cuadro de texto buscador, ignoramos la ejecución global.
    # Así el evento nativo del buscador procesará únicamente 1 salto en vez de 2.
    if event.widget == root and root.focus_get() == txt_buscar:
        return
    ...
    # Detener la acción nativa de Tkinter (mover cursor horizontal) si estamos en el Entry
    if event.widget == txt_buscar:
        return "break"
...
def asegurar_visibilidad_scroll(widget_tarjeta):
    ...
    try:
        ...
        frame_lista_global._parent_canvas.yview_moveto(y_widget / frame_lista_global.winfo_height())
    except Exception: pass
...
def iniciar_aplicacion():
    ...
    root = ctk.CTk()
    root.title("PromptVault Premium v36")
    ...
    # Binds directos al cuadro de búsqueda (Buscador)
    ...
    # Binds Globales asignados a la Ventana Principal

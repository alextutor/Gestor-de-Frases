...
def normalizar_y_validar_color(hex_str):
    ...
    if not hex_str:
        return "#000000" # Por defecto si está vacío
    
    # Agregar '#' si falta
    if not hex_str.startswith('#'):
        hex_str = '#' + hex_str
        
    # Validar formato #RGB o #RRGGBB usando Expresiones Regulares
...
def cargar_configuracion_global():
    ...
    try:
        ...
        for clave, valor in cursor.fetchall():
            if clave == "tema": 
                TEMA_ACTUAL = valor if valor in PALETAS else "Dark Neon Vault"
...
# ==========================================
# CRUD DE TEMAS COMERCIALES CORREGIDO
# ==========================================
class VentanaCrudTemas(ctk.CTkToplevel):
    def __init__(self, master, callback_recargar_menu, *args, **kwargs):
        ...
        def crear_input_color(parent, label_text, placeholder, clave_ref):
            ...
            btn = ctk.CTkButton(f, text="🎨", width=32, fg_color=t["btn_neutral"], command=lambda: self.invocar_selector_color(ent, clave_ref))
            btn.pack(side="right")
            return ent
        ...
        # BARRA DE CONTROL INFERIOR FIJA (BOTÓN SALIR AL LADO DERECHO)
        ...
        self.btn_salir = ctk.CTkButton(frame_barra_inferior, text="Salir", fg_color=t["btn_neutral"], command=self.destroy)
    ...
    def limpiar_formulario(self):
        ...
        for preview in self.visualizadores.values():
            preview.configure(fg_color="#1E293B")
    ...
    def guardar_tema(self):
        ...
        # Diccionario temporal para procesar y validar las entradas
        inputs = {
            "base": self.ent_base.get(),
            "texto": self.ent_texto.get(),
            "exito": self.ent_exito.get(),
            "accion": self.ent_accion.get(),
            "peligro": self.ent_peligro.get(),
            "neutral": self.ent_neutral.get(),
            "p_borde": self.ent_p_borde.get(),
            "p_bg": self.ent_p_bg.get(),
            "p_hover": self.ent_p_hover.get()
        }
        ...
        for clave, valor in inputs.items():
            ...
            if not validado:
                messagebox.showerror("Color Inválido", f"El color ingresado para '{clave}' no es un hexadecimal válido.")
                return
        ...
        if self.id_tema_seleccionado:
            ...
            ''', (nombre, valores_validados["base"], valores_validados["texto"], valores_validados["exito"], 
                  ...
                  borde_flag, bg_flag, valores_validados["p_borde"], valores_validados["p_bg"], valores_validados["p_hover"], self.id_tema_seleccionado))
        else:
            cursor.execute('''
        ...
        # PERSISTENCIA COMPLETA EN SESIÓN ACTUAL Y SIGUIENTES INICIOS
...
def iniciar_aplicacion():
    ...
    root = ctk.CTk()
    root.title("PromptVault Premium v29")
    
    # Aplicar centrado determinista al iniciar

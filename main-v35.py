...
INDICE_SELECCION_TECLADO = -1  # Control de navegación ▲▼
TARJETAS_VISUALES_CACHE = []   # Referencias a los widgets de tarjetas creadas
...
# ==========================================
# COMPONENTE VISUAL COLAPSABLE (UI REUTILIZABLE)
# ==========================================
class CollapsibleFrame(ctk.CTkFrame):
    def __init__(self, master, title, expanded=False, **kwargs):
        ...
        self.btn_toggle = ctk.CTkButton(
            ...
            command=self.toggle, anchor="w", fg_color="transparent",
            font=("Arial", 11, "bold")
        )
        self.btn_toggle.grid(row=0, column=0, sticky="ew", padx=5, pady=5)
    ...
    def toggle(self):
        if self.is_expanded:
            self.frame_content.grid_forget()
...
def inicializar_db_y_config():
    ...
    try:
        ...
        cursor.execute("INSERT OR IGNORE INTO categorias (nombre) VALUES ('General')")
        
        cursor.execute("PRAGMA table_info(prompts)")
        cols_prompts = [c[1] for c in cursor.fetchall()]
        if "categoria" not in cols_prompts:
            cursor.execute("ALTER TABLE prompts ADD COLUMN categoria TEXT DEFAULT 'General'")
        if "tags" not in cols_prompts:
        ...
        columnas_nuevas = {
            ...
            "color_titulo_sel": "'#06B6D4'",
            "prompt_color_bg_sel": "'#1E293B'",
            "buscar_text_color": "'#E2E8F0'",
            "buscar_fg_color": "'#111827'",
            "buscar_border_color": "'#10B981'"
        }
        ...
        cursor.execute("SELECT count(*) FROM temas_comerciales WHERE nombre='Dark Neon Vault'")
        if cursor.fetchone()[0] == 0:
            cursor.execute('''
                ...
                (nombre, modo, color_base, color_texto, btn_exito, btn_accion, btn_peligro, btn_neutral, 
                 ...
                 btn_exito_hover, btn_accion_hover, btn_peligro_hover, btn_neutral_hover, color_titulo_sel,
                 prompt_color_bg_sel, buscar_text_color, buscar_fg_color, buscar_border_color)
                VALUES 
                ('Dark Neon Vault', 'dark', '#0A0E17', '#E2E8F0', '#10B981', '#06B6D4', '#EF4444', '#4B5563', 
                 1, 1, '#10B981', '#111827', '#1F2937', '#059669', '#0891B2', '#DC2626', '#374151', '#06B6D4',
                 '#1E293B', '#E2E8F0', '#111827', '#10B981')
            ''')
        conexion.commit()
    ...
    try:
        ...
        for row in cursor.fetchall():
            PALETAS[row[0]] = {
                ...
                "color_titulo_sel": row[17] if row[17] else '#06B6D4',
                "prompt_color_bg_sel": row[18] if row[18] else '#1E293B',
                "buscar_text_color": row[19] if row[19] else '#E2E8F0',
                "buscar_fg_color": row[20] if row[20] else '#111827',
                "buscar_border_color": row[21] if row[21] else '#10B981'
            }
...
def actualizar_cache_prompts():
    ...
    try:
        conn = sqlite3.connect("prompts.sqlite")
        cursor = conn.cursor()
        cursor.execute("SELECT id, titulo, categoria, tags FROM prompts WHERE eliminado_en IS NULL ORDER BY id ASC")
        for fila in cursor.fetchall():
            PROMPTS_CACHE.append({
                "id": fila[0], 
                "etiqueta": fila[1], 
                "categoria": fila[2] if fila[2] else 'General',
                "tags": fila[3] if fila[3] else ''
            })
        conn.close()
    except Exception: pass
    recargar_combobox_categorias_ui()

def recargar_combobox_categorias_ui():
    if combo_categoria is None: return
    try:
        conn = sqlite3.connect("prompts.sqlite")
        cursor = conn.cursor()
    ...
    opciones = ["Todos"] + cats
    combo_categoria.configure(values=opciones)

def obtener_lista_categorias_raw():
    try:
        conn = sqlite3.connect("prompts.sqlite")
        cursor = conn.cursor()
        cursor.execute("SELECT nombre FROM categorias ORDER BY nombre ASC")
...
def guardar_nuevo_prompt(titulo, contenido, categoria, tags):
    try:
        conn = sqlite3.connect("prompts.sqlite")
        cursor = conn.cursor()
        cursor.execute("INSERT INTO prompts (titulo, contenido, categoria, tags) VALUES (?, ?, ?, ?)", (titulo, contenido, categoria, tags))
        conn.commit()
        conn.close()
    except Exception: pass

def actualizar_prompt(prompt_id, titulo, contenido, categoria, tags):
    try:
        conn = sqlite3.connect("prompts.sqlite")
        cursor = conn.cursor()
        cursor.execute("UPDATE prompts SET titulo = ?, contenido = ?, categoria = ?, tags = ? WHERE id = ?", (titulo, contenido, categoria, tags, prompt_id))
        conn.commit()
        conn.close()
    except Exception: pass
...
class VentanaFormulario(ctk.CTkToplevel):
    def __init__(self, master, prompt_id=None, *args, **kwargs):
        ...
        frame_meta = ctk.CTkFrame(self, fg_color="transparent")
        frame_meta.pack(pady=(15, 2), padx=20, fill="x")
        
        frame_cat = ctk.CTkFrame(frame_meta, fg_color="transparent")
        frame_cat.pack(side="left", fill="x", expand=True, padx=(0,5))
        ctk.CTkLabel(frame_cat, text="Categoría:", text_color=t["color_texto"], font=("Arial", 11, "bold")).pack(anchor="w")
        
        self.combo_form_cat = ctk.CTkComboBox(frame_cat, values=obtener_lista_categorias_raw(), state="readonly")
        self.combo_form_cat.pack(fill="x", pady=2)
        self.combo_form_cat.set("General")
        
        frame_tags = ctk.CTkFrame(frame_meta, fg_color="transparent")
        frame_tags.pack(side="right", fill="x", expand=True, padx=(5,0))
        ctk.CTkLabel(frame_tags, text="Etiquetas / Tags (separados por coma):", text_color=t["color_texto"], font=("Arial", 11, "bold")).pack(anchor="w")
        self.txt_tags = ctk.CTkEntry(frame_tags, placeholder_text="Ej: php, sql, ia")
        self.txt_tags.pack(fill="x", pady=2)

        ctk.CTkLabel(self, text="Título del Prompt:", text_color=t["color_texto"], font=("Arial", 12, "bold")).pack(pady=(10, 2), padx=20, anchor="w")
        self.txt_titulo = ctk.CTkEntry(self)
        self.txt_titulo.pack(pady=5, padx=20, fill="x")
        
        ctk.CTkLabel(self, text="Contenido del Prompt:", text_color=t["color_texto"], font=("Arial", 12, "bold")).pack(pady=(10, 2), padx=20, anchor="w")
        self.txt_contenido = ctk.CTkTextbox(self, height=240)
        self.txt_contenido.pack(pady=5, padx=20, fill="both", expand=True)
        
        frame_btns = ctk.CTkFrame(self, fg_color="transparent")
        frame_btns.pack(fill="x", pady=20, padx=20)
        
        btn_s = ctk.CTkButton(frame_btns, text="Salir", fg_color=t["btn_neutral"], hover_color=t["btn_neutral_hover"], width=120, command=self.destroy)
        btn_s.pack(side="left", padx=(0, 10))
        
        btn_g = ctk.CTkButton(frame_btns, text="Guardar Prompt", fg_color=t["btn_exito"], hover_color=t["btn_exito_hover"], command=self.procesar_datos)
        btn_g.pack(side="right", fill="x", expand=True)
        
        if self.prompt_id:
            self.cargar_datos_existentes()

    def cargar_datos_existentes(self):
        item_cache = next((r for r in PROMPTS_CACHE if r["id"] == self.prompt_id), None)
        contenido_actual = obtener_texto_prompt(self.prompt_id)
        if item_cache:
            self.txt_titulo.insert(0, item_cache["etiqueta"])
            if item_cache["categoria"] in self.combo_form_cat.cget("values"):
                self.combo_form_cat.set(item_cache["categoria"])
            else:
                self.combo_form_cat.set("General")
            self.txt_tags.insert(0, item_cache["tags"])
    ...
    def procesar_datos(self):
        ...
        categoria = self.combo_form_cat.get()
        tags = self.txt_tags.get().strip()
        
        if not titulo:
            messagebox.showwarning("Falta Información", "Por favor, introduzca un título válido para el prompt.")
            self.txt_titulo.focus_set()
            return
        if not contenido:
            messagebox.showwarning("Falta Información", "El contenido del prompt no puede estar completamente vacío.")
            self.txt_contenido.focus_set()
            return
            
        if self.prompt_id:
            actualizar_prompt(self.prompt_id, titulo, contenido, categoria, tags)
        else:
            guardar_nuevo_prompt(titulo, contenido, categoria, tags)
...
# CRUD DE CATEGORÍAS
# ==========================================
class VentanaCrudCategorias(ctk.CTkToplevel):
    def __init__(self, master, *args, **kwargs):
        super().__init__(master, *args, **kwargs)
        self.title("Administrar Categorías v35")
        self.transient(master)
        self.grab_set()
        
        centrar_ventana(self, 520, 440)
        self.bind("<Escape>", lambda e: self.destroy())
        ...
        ctk.CTkLabel(self, text="Mantenimiento de Categorías", text_color=t["color_texto"], font=("Arial", 14, "bold")).pack(pady=10)
        
        frame_cuerpo = ctk.CTkFrame(self, fg_color="transparent")
        frame_cuerpo.pack(fill="both", expand=True, padx=15, pady=5)
        
        estilo = ttk.Style()
        estilo.theme_use("clam")
        estilo.configure("Cat.Treeview", background="#1E293B", foreground="white", fieldbackground="#1E293B", rowheight=24)
        
        self.tabla_cat = ttk.Treeview(frame_cuerpo, columns=("Nombre"), show="headings", style="Cat.Treeview")
        self.tabla_cat.heading("Nombre", text="Nombre de Categoría")
        self.tabla_cat.column("Nombre", width=280)
        self.tabla_cat.pack(side="left", fill="both", expand=True, padx=(0, 10))
        self.tabla_cat.bind("<<TreeviewSelect>>", self.al_seleccionar_categoria)
        
        frame_form = ctk.CTkFrame(frame_cuerpo, fg_color="transparent", width=180)
        frame_form.pack(side="right", fill="both")
        
        ctk.CTkLabel(frame_form, text="Nombre Ficha:", text_color=t["color_texto"], font=("Arial", 11, "bold")).pack(anchor="w")
        self.ent_cat_nombre = ctk.CTkEntry(frame_form)
        self.ent_cat_nombre.pack(fill="x", pady=4)
        
        self.btn_guardar_cat = ctk.CTkButton(frame_form, text="Añadir", fg_color=t["btn_exito"], hover_color=t["btn_exito_hover"], command=self.guardar_categoria)
        self.btn_guardar_cat.pack(fill="x", pady=4)
        
        self.btn_limpiar_cat = ctk.CTkButton(frame_form, text="Limpiar", fg_color=t["btn_neutral"], hover_color=t["btn_neutral_hover"], command=self.limpiar_campos)
        self.btn_limpiar_cat.pack(fill="x", pady=4)
        
        self.btn_eliminar_cat = ctk.CTkButton(frame_form, text="Eliminar", fg_color=t["btn_peligro"], hover_color=t["btn_peligro_hover"], command=self.eliminar_categoria)
        self.btn_eliminar_cat.pack(fill="x", pady=4)
        self.btn_eliminar_cat.configure(state="disabled")
        
        frame_baja = ctk.CTkFrame(self, fg_color="transparent")
        frame_baja.pack(fill="x", side="bottom", pady=15, padx=15)
        
        btn_salir = ctk.CTkButton(frame_baja, text="Salir", fg_color=t["btn_neutral"], hover_color=t["btn_neutral_hover"], command=self.destroy)
        btn_salir.pack(side="right")
        
        self.actualizar_tabla_categorias()

    def actualizar_tabla_categorias(self):
        for item in self.tabla_cat.get_children(): self.tabla_cat.delete(item)
        lista = obtener_lista_categorias_raw()
        for cat in lista:
            self.tabla_cat.insert("", "end", iid=cat, values=(cat,))

    def al_seleccionar_categoria(self, event):
        sel = self.tabla_cat.selection()
        if not sel: return
        self.categoria_seleccionada = sel[0]
        self.ent_cat_nombre.delete(0, tk.END)
        self.ent_cat_nombre.insert(0, self.categoria_seleccionada)
        
        self.btn_guardar_cat.configure(text="Modificar")
        if self.categoria_seleccionada != "General":
            self.btn_eliminar_cat.configure(state="normal")
        else:
            self.btn_eliminar_cat.configure(state="disabled")
    ...
    def limpiar_campos(self):
        self.categoria_seleccionada = None
        self.ent_cat_nombre.delete(0, tk.END)
        self.btn_guardar_cat.configure(text="Añadir")
        self.btn_eliminar_cat.configure(state="disabled")
        self.tabla_cat.selection_remove(self.tabla_cat.selection())

    def guardar_categoria(self):
        nuevo_nombre = self.ent_cat_nombre.get().strip()
        if not nuevo_nombre:
            messagebox.showwarning("Validación", "El nombre de la categoría es obligatorio.")
            return
            
        if nuevo_nombre.lower() == "todos":
            messagebox.showwarning("Restricción", "No se permite ingresar la categoría 'Todos' porque ya existe por defecto en el sistema.")
            return
        ...
        try:
            ...
            if self.categoria_seleccionada:
                if self.categoria_seleccionada == "General":
                    messagebox.showwarning("Restricción", "No se permite modificar la categoría raíz 'General'.")
                    conn.close()
                    return
                cursor.execute("INSERT OR IGNORE INTO categorias (nombre) VALUES (?)", (nuevo_nombre,))
                cursor.execute("UPDATE prompts SET categoria = ? WHERE categoria = ?", (nuevo_nombre, self.categoria_seleccionada))
                cursor.execute("DELETE FROM categorias WHERE nombre = ?", (self.categoria_seleccionada,))
            else:
                cursor.execute("INSERT OR IGNORE INTO categorias (nombre) VALUES (?)", (nuevo_nombre,))
    ...
    def eliminar_categoria(self):
        if not self.categoria_seleccionada or self.categoria_seleccionada == "General": return
        
        try:
            conn = sqlite3.connect("prompts.sqlite")
            cursor = conn.cursor()
            cursor.execute("UPDATE prompts SET categoria = 'General' WHERE categoria = ?", (self.categoria_seleccionada,))
            cursor.execute("DELETE FROM categorias WHERE nombre = ?", (self.categoria_seleccionada,))
            conn.commit()
            conn.close()
        except Exception: pass
...
# ==========================================
# CRUD DE TEMAS AVANZADO
# ==========================================
class VentanaCrudTemas(ctk.CTkToplevel):
    def __init__(self, callback_recargar_menu, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.callback_recargar_menu = callback_recargar_menu
        self.title("Administrar Temas Avanzado v35")
        ...
        frame_scroll_grupos = ctk.CTkScrollableFrame(panel_izquierdo)
        frame_scroll_grupos.pack(fill="both", expand=True, pady=(0, 10))
        
        def crear_input_color_modular(parent, label_text, placeholder, clave_ref):
            ctk.CTkLabel(parent, text=label_text).pack(anchor="w", padx=10, pady=(4,0))
            ...
            btn = ctk.CTkButton(f, text="🎨", width=34, fg_color=t["btn_neutral"], hover_color=t["btn_neutral_hover"], command=lambda: self.invocar_selector_color(ent, clave_ref))
            btn.pack(side="right")
            return ent

        self.grupo1 = CollapsibleFrame(frame_scroll_grupos, title="Grupo 1: Identidad Base y Textos", expanded=True)
        self.grupo1.pack(fill="x", padx=5, pady=5)
        
        ctk.CTkLabel(self.grupo1.frame_content, text="Nombre del Tema comercial:").pack(anchor="w", padx=10, pady=(2,0))
        self.ent_nombre = ctk.CTkEntry(self.grupo1.frame_content)
        self.ent_nombre.pack(fill="x", padx=10, pady=2)
        
        self.ent_base = crear_input_color_modular(self.grupo1.frame_content, "Color Base (Ventana Principal):", "#0A0E17", "base")
        self.ent_texto = crear_input_color_modular(self.grupo1.frame_content, "Color Texto General:", "#E2E8F0", "texto")

        self.grupo2 = CollapsibleFrame(frame_scroll_grupos, title="Grupo 2: Colores Funcionales y Hovers", expanded=False)
        self.grupo2.pack(fill="x", padx=5, pady=5)
        
        self.ent_exito = crear_input_color_modular(self.grupo2.frame_content, "Color Éxito (Botón Guardar/Nuevo):", "#10B981", "exito")
        self.ent_exito_hover = crear_input_color_modular(self.grupo2.frame_content, "↳ Hover Éxito:", "#059669", "exito_hover")
        
        self.ent_accion = crear_input_color_modular(self.grupo2.frame_content, "Color Acción (Inyección):", "#06B6D4", "accion")
        self.ent_accion_hover = crear_input_color_modular(self.grupo2.frame_content, "↳ Hover Acción:", "#0891B2", "accion_hover")
        
        self.ent_peligro = crear_input_color_modular(self.grupo2.frame_content, "Color Peligro (Eliminaciones):", "#EF4444", "peligro")
        self.ent_peligro_hover = crear_input_color_modular(self.grupo2.frame_content, "↳ Hover Peligro:", "#DC2626", "peligro_hover")
        
        self.ent_neutral = crear_input_color_modular(self.grupo2.frame_content, "Color Neutral (Utilidades):", "#4B5563", "neutral")
        self.ent_neutral_hover = crear_input_color_modular(self.grupo2.frame_content, "↳ Hover Neutral:", "#374151", "neutral_hover")

        self.grupo3 = CollapsibleFrame(frame_scroll_grupos, title="Grupo 3: Estética, Diseño de Tarjetas y Buscador", expanded=False)
        self.grupo3.pack(fill="x", padx=5, pady=5)
        
        self.chk_borde = ctk.CTkCheckBox(self.grupo3.frame_content, text="Dibujar Bordes Activos")
        self.chk_borde.pack(anchor="w", padx=10, pady=4)
        
        self.chk_bg = ctk.CTkCheckBox(self.grupo3.frame_content, text="Rellenar Fondo de Tarjeta")
        self.chk_bg.pack(anchor="w", padx=10, pady=4)
        
        self.ent_p_borde = crear_input_color_modular(self.grupo3.frame_content, "Color del Borde Tarjeta:", "#10B981", "p_borde")
        self.ent_p_bg = crear_input_color_modular(self.grupo3.frame_content, "Color de Fondo Tarjeta Pasiva:", "#111827", "p_bg")
        self.ent_p_hover = crear_input_color_modular(self.grupo3.frame_content, "Color de Reacción Hover Tarjeta:", "#1F2937", "p_hover")
        
        self.ent_titulo_sel = crear_input_color_modular(self.grupo3.frame_content, "Color Título Prompt Seleccionado:", "#06B6D4", "titulo_sel")
        self.ent_p_bg_sel = crear_input_color_modular(self.grupo3.frame_content, "Color Fondo Prompt Seleccionado:", "#1E293B", "p_bg_sel")
        
        self.ent_buscar_txt = crear_input_color_modular(self.grupo3.frame_content, "Buscador: Color del Texto:", "#E2E8F0", "buscar_txt")
        self.ent_buscar_bg = crear_input_color_modular(self.grupo3.frame_content, "Buscador: Color del Fondo:", "#111827", "buscar_bg")
        self.ent_buscar_border = crear_input_color_modular(self.grupo3.frame_content, "Buscador: Color del Contorno:", "#10B981", "buscar_border")

        self.btn_guardar = ctk.CTkButton(panel_izquierdo, text="Registrar / Guardar Tema", fg_color=t["btn_exito"], hover_color=t["btn_exito_hover"], command=self.guardar_tema)
        self.btn_guardar.pack(fill="x", pady=5)
        
        btn_l = ctk.CTkButton(panel_izquierdo, text="Limpiar Ficha", fg_color=t["btn_neutral"], hover_color=t["btn_neutral_hover"], command=self.limpiar_formulario)
        btn_l.pack(fill="x", pady=2)
        
        self.btn_eliminar = ctk.CTkButton(panel_izquierdo, text="Destruir Tema", fg_color=t["btn_peligro"], hover_color=t["btn_peligro_hover"], command=self.eliminar_tema)
        self.btn_eliminar.pack(fill="x", pady=2)
        ...
        self.btn_salir = ctk.CTkButton(frame_barra_inferior, text="Cerrar", fg_color=t["btn_neutral"], hover_color=t["btn_neutral_hover"], command=self.destroy)
        self.btn_salir.pack(side="right")
        
        frame_lista = ctk.CTkFrame(self)
        frame_lista.grid(row=0, column=1, sticky="nsew", padx=10, pady=10)
        
        ctk.CTkLabel(frame_lista, text="Esquemas Disponibles en Disco", font=("Arial", 12, "bold")).pack(pady=10)
    ...
    def actualizar_muestra_desde_texto(self, clave_ref):
        try:
            ...
            elif clave_ref == "titulo_sel": val_txt = self.ent_titulo_sel.get()
            elif clave_ref == "p_bg_sel": val_txt = self.ent_p_bg_sel.get()
            elif clave_ref == "buscar_txt": val_txt = self.ent_buscar_txt.get()
            elif clave_ref == "buscar_bg": val_txt = self.ent_buscar_bg.get()
            elif clave_ref == "buscar_border": val_txt = self.ent_buscar_border.get()
    ...
    def al_seleccionar_registro(self, event):
        ...
        if r:
            ...
            self.ent_titulo_sel.delete(0, tk.END); self.ent_titulo_sel.insert(0, r[18] if len(r) > 18 and r[18] else '#06B6D4')
            self.ent_p_bg_sel.delete(0, tk.END); self.ent_p_bg_sel.insert(0, r[19] if len(r) > 19 and r[19] else '#1E293B')
            self.ent_buscar_txt.delete(0, tk.END); self.ent_buscar_txt.insert(0, r[20] if len(r) > 20 and r[20] else '#E2E8F0')
            self.ent_buscar_bg.delete(0, tk.END); self.ent_buscar_bg.insert(0, r[21] if len(r) > 21 and r[21] else '#111827')
            self.ent_buscar_border.delete(0, tk.END); self.ent_buscar_border.insert(0, r[22] if len(r) > 22 and r[22] else '#10B981')
    ...
    def limpiar_formulario(self):
        ...
        self.ent_titulo_sel.delete(0, tk.END)
        self.ent_p_bg_sel.delete(0, tk.END)
        self.ent_buscar_txt.delete(0, tk.END)
        self.ent_buscar_bg.delete(0, tk.END)
        self.ent_buscar_border.delete(0, tk.END)
    ...
    def guardar_tema(self):
        ...
        inputs = {
            "base": self.ent_base.get(), "texto": self.ent_texto.get(), "titulo_sel": self.ent_titulo_sel.get(),
            "p_bg_sel": self.ent_p_bg_sel.get(), "buscar_txt": self.ent_buscar_txt.get(), "buscar_bg": self.ent_buscar_bg.get(),
            "buscar_border": self.ent_buscar_border.get(),
            "exito": self.ent_exito.get(), "exito_hover": self.ent_exito_hover.get(), "accion": self.ent_accion.get(), 
            "accion_hover": self.ent_accion_hover.get(), "peligro": self.ent_peligro.get(), "peligro_hover": self.ent_peligro_hover.get(), 
            "neutral": self.ent_neutral.get(), "neutral_hover": self.ent_neutral_hover.get(), "p_borde": self.ent_p_borde.get(), 
            "p_bg": self.ent_p_bg.get(), "p_hover": self.ent_p_hover.get()
        }
        ...
        if self.id_tema_seleccionado:
            cursor.execute('''
                UPDATE temas_comerciales SET nombre=?, color_base=?, color_texto=?, btn_exito=?, btn_accion=?, 
                                             ...
                                             color_titulo_sel=?, prompt_color_bg_sel=?, buscar_text_color=?, 
                                             buscar_fg_color=?, buscar_border_color=? WHERE id=?
            ''', (nombre, valores_validados["base"], valores_validados["texto"], valores_validados["exito"], 
                  ...
                  valores_validados["exito_hover"], valores_validados["accion_hover"], valores_validados["peligro_hover"], 
                  valores_validados["neutral_hover"], valores_validados["titulo_sel"], valores_validados["p_bg_sel"],
                  valores_validados["buscar_txt"], valores_validados["buscar_bg"], valores_validados["buscar_border"],
                  self.id_tema_seleccionado))
        else:
            cursor.execute('''
                ...
                (nombre, modo, color_base, color_texto, btn_exito, btn_accion, btn_peligro, btn_neutral, prompt_usar_borde, 
                 prompt_usar_bg, prompt_color_borde, prompt_color_bg, prompt_color_hover, btn_exito_hover, btn_accion_hover, 
                 btn_peligro_hover, btn_neutral_hover, color_titulo_sel, prompt_color_bg_sel, buscar_text_color, 
                 buscar_fg_color, buscar_border_color)
                VALUES (?, 'dark', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ''', (nombre, valores_validados["base"], valores_validados["texto"], valores_validados["exito"], 
                  valores_validados["accion"], valores_validados["peligro"], valores_validados["neutral"], 
                  borde_flag, bg_flag, valores_validados["p_borde"], valores_validados["p_bg"], valores_validados["p_hover"],
...
# ==========================================
# INTERFAZ Y LOGICA DE NAVEGACIÓN v35
# ==========================================
def gestionar_navegacion_teclado(event):
    """ Módulo centralizado para el control ▲, ▼ y Enter """
    global INDICE_SELECCION_TECLADO, id_prompt_actual
    cantidad_tarjetas = len(TARJETAS_VISUALES_CACHE)
    if cantidad_tarjetas == 0: return

    if event.keysym == "Down":
        if INDICE_SELECCION_TECLADO < cantidad_tarjetas - 1:
            INDICE_SELECCION_TECLADO += 1
        else:
            INDICE_SELECCION_TECLADO = 0  # Bucle al inicio
    elif event.keysym == "Up":
        if INDICE_SELECCION_TECLADO > 0:
            INDICE_SELECCION_TECLADO -= 1
        else:
            INDICE_SELECCION_TECLADO = cantidad_tarjetas - 1  # Bucle al final

    # Aplicar efectos visuales y actualizar selección
    meta_tarjeta = TARJETAS_VISUALES_CACHE[INDICE_SELECCION_TECLADO]
    seleccionar_prompt(meta_tarjeta["id"], meta_tarjeta["frame"], frame_lista_global, actualizar_indice=False)
    
    # Módulo de Scroll Automático Inteligente
    asegurar_visibilidad_scroll(meta_tarjeta["frame"])

def asegurar_visibilidad_scroll(widget_tarjeta):
    """ Fuerza al ScrollableFrame a deslizarse hacia la tarjeta activa """
    try:
        frame_lista_global.update_idletasks()
        # Obtener posiciones relativas
        y_widget = widget_tarjeta.winfo_y()
        h_widget = widget_tarjeta.winfo_height()
        
        # Obtener dimensiones internas visibles de la lista
        h_contenedor = frame_lista_global._parent_canvas.winfo_height()
        
        # Calcular posición fraccional para el canvas
        scroll_top = y_widget
        scroll_bottom = y_widget + h_widget
        
        # Mover barra si está fuera del margen de visión
        frame_lista_global._parent_canvas.yview_moveto(scroll_top / frame_lista_global.winfo_height())
    except Exception: pass

def ejecutar_enter_teclado(event):
    """ Enlace directo de Enter para disparar la inyección sin mouse """
    if id_prompt_actual is not None:
        ejecutar_disparo_dinamico(id_prompt_actual, ocu_v=True)

def seleccionar_prompt(id_seleccionado, frame_asociado, frame_contenedor, actualizar_indice=True):
    global id_prompt_actual, INDICE_SELECCION_TECLADO
    id_prompt_actual = id_seleccionado
    t = PALETAS[TEMA_ACTUAL]
    
    # Sincronizar índice numérico de teclado si la selección provino de un clic físico
    if actualizar_indice:
        for idx, tarjeta in enumerate(TARJETAS_VISUALES_CACHE):
            if tarjeta["id"] == id_seleccionado:
                INDICE_SELECCION_TECLADO = idx
                break
    ...
    for child in frame_contenedor.winfo_children():
        if isinstance(child, ctk.CTkFrame):
            ...
            for widget in child.winfo_children():
                if isinstance(widget, ctk.CTkFrame): # Contenedor interno de información
                    widget.configure(fg_color="transparent")
                    for sub_w in widget.winfo_children():
                        if isinstance(sub_w, ctk.CTkLabel) and sub_w.cget("text") not in ["✕", "✎"]:
                            sub_w.configure(text_color=t["color_texto"])
                if isinstance(widget, ctk.CTkLabel) and widget.cget("text") not in ["✕", "✎"]:
                    widget.configure(text_color=t["color_texto"])
            
    card_bg_sel = t.get("prompt_color_bg_sel", "#1E293B")
    frame_asociado.configure(fg_color=card_bg_sel)
    
    # Pintar títulos internos de la tarjeta seleccionada
    for widget in frame_asociado.winfo_children():
        if isinstance(widget, ctk.CTkFrame):
            for sub_w in widget.winfo_children():
                if isinstance(sub_w, ctk.CTkLabel) and sub_w.cget("text") not in ["✕", "✎"]:
                    sub_w.configure(text_color=t.get("color_titulo_sel", t["prompt_color_borde"]))
        if isinstance(widget, ctk.CTkLabel) and widget.cget("text") not in ["✕", "✎"]:
            widget.configure(text_color=t.get("color_titulo_sel", t["prompt_color_borde"]))
...
def filtrar_prompts_ui(event=None):
    global frame_lista_global, txt_buscar, combo_categoria, TARJETAS_VISUALES_CACHE, INDICE_SELECCION_TECLADO
    if frame_lista_global is None: return
    
    for widget in frame_lista_global.winfo_children(): widget.destroy()
    TARJETAS_VISUALES_CACHE = []
    
    texto_busqueda = txt_buscar.get().strip().lower() if txt_buscar else ""
    cat_seleccionada = combo_categoria.get() if combo_categoria else "Todos"
    
    t = PALETAS[TEMA_ACTUAL]
    resultados = []
    
    for p in PROMPTS_CACHE:
        if cat_seleccionada != "Todos" and p["categoria"] != cat_seleccionada:
            continue
        if texto_busqueda and (texto_busqueda not in p["etiqueta"].lower() and texto_busqueda not in p["tags"].lower()):
            continue
        resultados.append(p)
    ...
    for reg in resultados:
        ...
        color_inicial = t.get("prompt_color_bg_sel", "#1E293B") if id_prompt_actual == reg_id else card_bg
        color_texto_inicial = t.get("color_titulo_sel", t["prompt_color_borde"]) if id_prompt_actual == reg_id else t["color_texto"]
        ...
        frame_info_bloque = ctk.CTkFrame(card_frame, fg_color="transparent")
        frame_info_bloque.pack(side="left", fill="x", expand=True, padx=12, pady=6)
        
        lbl_titulo = ctk.CTkLabel(frame_info_bloque, text=reg_titulo, text_color=color_texto_inicial, font=("Arial", 13, "bold"), anchor="w", cursor="hand2")
        lbl_titulo.pack(fill="x", anchor="w")
        
        meta_str = f"Categoría: {reg_cat}"
        if reg_tags: meta_str += f"  |  Tags: {reg_tags}"
        lbl_meta = ctk.CTkLabel(frame_info_bloque, text=meta_str, text_color="gray", font=("Arial", 10, "italic"), anchor="w")
        lbl_meta.pack(fill="x", anchor="w", pady=(2, 0))
        
        # Conservar referencias ordenadas para mapeo de teclas ▲ y ▼
        TARJETAS_VISUALES_CACHE.append({"id": reg_id, "frame": card_frame})

        # ==========================================
        # UNIFICACIÓN DE EVENTOS (MÓDULO HOVER HOMOGÉNEO)
        # ==========================================
        def al_entrar_mouse(e, frm=card_frame, lbl=lbl_titulo, hover_c=card_hover, r_id=reg_id):
            ...
            if id_prompt_actual != r_id:
                lbl.configure(text_color=t.get("color_titulo_sel", t["prompt_color_borde"]))

        def al_salir_mouse(e, r_id=reg_id, frm=card_frame, lbl=lbl_titulo, def_bg=card_bg):
            if id_prompt_actual == r_id:
                frm.configure(fg_color=t.get("prompt_color_bg_sel", "#1E293B"))
                lbl.configure(text_color=t.get("color_titulo_sel", t["prompt_color_borde"]))
            else:
                frm.configure(fg_color=def_bg)
                lbl.configure(text_color=t["color_texto"])

        # Enlazar todos los sub-componentes al mismo manejador unificado para eliminar la asimetría de entrada
        card_frame.bind("<Enter>", al_entrar_mouse)
        card_frame.bind("<Leave>", al_salir_mouse)
        frame_info_bloque.bind("<Enter>", al_entrar_mouse)
        frame_info_bloque.bind("<Leave>", al_salir_mouse)
        lbl_titulo.bind("<Enter>", al_entrar_mouse)
        lbl_titulo.bind("<Leave>", al_salir_mouse)
        lbl_meta.bind("<Enter>", al_entrar_mouse)
        lbl_meta.bind("<Leave>", al_salir_mouse)
        
        # Enlaces de Clic para Selección
        card_frame.bind("<Button-1>", lambda e, r_id=reg_id, f=card_frame: seleccionar_prompt(r_id, f, frame_lista_global))
        frame_info_bloque.bind("<Button-1>", lambda e, r_id=reg_id, f=card_frame: seleccionar_prompt(r_id, f, frame_lista_global))
        lbl_titulo.bind("<Button-1>", lambda e, r_id=reg_id, f=card_frame: seleccionar_prompt(r_id, f, frame_lista_global))
        lbl_meta.bind("<Button-1>", lambda e, r_id=reg_id, f=card_frame: seleccionar_prompt(r_id, f, frame_lista_global))
        
        # Enlaces de Doble Clic para Inyección Dinámica
        card_frame.bind("<Double-Button-1>", lambda e, r_id=reg_id: ejecutar_disparo_dinamico(r_id, ocu_v=True))
        frame_info_bloque.bind("<Double-Button-1>", lambda e, r_id=reg_id: ejecutar_disparo_dinamico(r_id, ocu_v=True))
        lbl_titulo.bind("<Double-Button-1>", lambda e, r_id=reg_id: ejecutar_disparo_dinamico(r_id, ocu_v=True))
        
        btn_edit = ctk.CTkButton(card_frame, text="✎", width=34, height=28, fg_color=t["btn_neutral"], hover_color=t["btn_neutral_hover"],
                                 command=lambda r_id=reg_id: abrir_editor_directo(r_id))
        btn_edit.pack(side="left", padx=2)
        
        btn_del = ctk.CTkButton(card_frame, text="✕", width=34, height=28, fg_color=t["btn_peligro"], hover_color=t["btn_peligro_hover"],
                                command=lambda r_id=reg_id, t_p=reg_titulo: abrir_confirmacion_directa(r_id, t_p))
        btn_del.pack(side="left", padx=(2, 8))

    # Re-sincronizar índice activo tras un refresco de filtros
    if id_prompt_actual is not None:
        for idx, tarjeta in enumerate(TARJETAS_VISUALES_CACHE):
            if tarjeta["id"] == id_prompt_actual:
                INDICE_SELECCION_TECLADO = idx
                break
    else:
        INDICE_SELECCION_TECLADO = -1
...
def cambiar_tema_aplicacion(nombre_tema):
    if nombre_tema in PALETAS:
        ...
        if root:
            ...
            cfg_txt = t.get("buscar_text_color", "#E2E8F0")
            cfg_bg = t.get("buscar_fg_color", "#111827")
            cfg_border = t.get("buscar_border_color", "#10B981")
            
            if txt_buscar:
                txt_buscar.configure(text_color=cfg_txt, fg_color=cfg_bg, border_color=cfg_border)
            if combo_categoria:
                combo_categoria.configure(text_color=cfg_txt, fg_color=cfg_bg, border_color=cfg_border, button_color=cfg_border)
            if btn_lupa:
                btn_lupa.configure(fg_color=cfg_bg, border_color=cfg_border, text_color=cfg_txt)
...
def iniciar_aplicacion():
    global root, frame_lista_global, txt_buscar, combo_categoria, btn_lupa, submenu_temas, frame_izq, btn_nuevo, btn_inyectar, lbl_header_global
    ...
    root = ctk.CTk()
    root.title("PromptVault Premium v35")
    
    centrar_ventana(root, 750, 560)
    ...
    menu_config.add_command(label="Administrar Temas...", command=lambda: VentanaCrudTemas(recargar_menu_temas_dinamico, master=root))
    menu_config.add_separator()
    menu_config.add_command(label="Administrar Categorías...", command=lambda: VentanaCrudCategorias(root))
    ...
    frame_filtros = ctk.CTkFrame(frame_izq, fg_color="transparent")
    ...
    combo_categoria = ctk.CTkComboBox(
        frame_filtros, 
        width=140, 
        command=lambda v: filtrar_prompts_ui(),
        text_color=cfg_txt,
        fg_color=cfg_bg,
        border_color=cfg_border,
        button_color=cfg_border,
        state="readonly"
    )
    combo_categoria.pack(side="left", padx=(0, 6))
    
    txt_buscar = ctk.CTkEntry(
        frame_filtros, 
        placeholder_text="Buscar por título o etiquetas (tags)...",
        text_color=cfg_txt,
        fg_color=cfg_bg,
        border_color=cfg_border
    )
    txt_buscar.pack(side="left", fill="x", expand=True)
    txt_buscar.bind("<KeyRelease>", filtrar_prompts_ui)
    
    # Binds de navegación para el cuadro de búsqueda (Foco y transferencia)
    txt_buscar.bind("<Down>", gestionar_navegacion_teclado)
    txt_buscar.bind("<Up>", gestionar_navegacion_teclado)
    txt_buscar.bind("<Return>", ejecutar_enter_teclado)
    ...
    btn_lupa = ctk.CTkButton(
        ...
        text_color=cfg_txt,
        font=("Arial", 12, "bold"),
        command=filtrar_prompts_ui
    )
    btn_lupa.pack(side="left", padx=(6, 0))
    ...
    btn_nuevo = ctk.CTkButton(frame_acciones, text="[+] Nuevo Prompt", fg_color=t["btn_exito"], hover_color=t["btn_exito_hover"], width=150, command=abrir_creador)
    ...
    # Binds Globales de la Ventana Principal
    root.bind("<Down>", gestionar_navegacion_teclado)
    root.bind("<Up>", gestionar_navegacion_teclado)
    root.bind("<Return>", ejecutar_enter_teclado)

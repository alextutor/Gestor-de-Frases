...
# --- FUNCIÓN GLOBAL DE POSICIONAMIENTO ---
def centrar_ventana(ventana, ancho, alto):
    ...
    # Aplicar la geometría: "ancho x alto + x + y"
...
class VentanaVariables(ctk.CTkToplevel):
    def __init__(self, parent, datos_prompt, callback_actualizar):
        ...
        # Centramos con respecto a la pantalla
...
class PromptVaultApp(ctk.CTk):
    def __init__(self):
        super().__init__()
        ancho, alto = 480, 600
        self.title("PromptVault v1.1")
        
        # Centrar la principal al iniciar
        centrar_ventana(self, ancho, alto)
    ...
    def actualizar_ui_lista(self, query):
        ...
        for row in cursor.fetchall():
            f = ctk.CTkFrame(self.scroll_lista, fg_color="transparent")
            f.pack(fill="x", pady=2)
            btn = ctk.CTkButton(f, text=row[1], height=45, corner_radius=12,
                               fg_color="#1e1e1e", hover_color="#2a2a2a", anchor="w",

...
def inicializar_db_y_config():
    ...
    try:
        ...
        columnas_nuevas = {
            ...
            "prompt_color_bg_sel": "'#1E293B'", # v32: Fondo Seleccionado
            "buscar_text_color": "'#E2E8F0'",    # v32: Texto Buscador
            "buscar_fg_color": "'#111827'",      # v32: Fondo Buscador
            "buscar_border_color": "'#10B981'"   # v32: Contorno Buscador
        }
...
# ==========================================
# CRUD DE TEMAS COMERCIALES AVANZADO v32
# ==========================================
class VentanaCrudTemas(ctk.CTkToplevel):
    def __init__(self, callback_recargar_menu, *args, **kwargs):
        ...
        self.title("Administrar Temas Avanzado v32")
        ...
        # -------------------------------------------------------------
        # GRUPO 1: CONFIGURACIÓN BASE & COMPORTAMIENTO TEXTO (EXPANDIDO)
        # -------------------------------------------------------------
        ...
        # -------------------------------------------------------------
        # GRUPO 2: ATÓMICA DE ESTADOS Y ACCIONES
        # -------------------------------------------------------------
        ...
        # -------------------------------------------------------------
        # GRUPO 3: DISEÑO ESTRUCTURAL DE LISTADO / TARJETAS / BUSCADOR (v32)
        # -------------------------------------------------------------
        ...
        # v32: Tarjeta Seleccionada (Reubicado y Nuevo Fondo)
        ...
        # v32: Estilo Campo Buscar Prompt
        ...
        # PANEL ACCIONES DE LA VENTANA
        self.btn_guardar = ctk.CTkButton(panel_izquierdo, text="Registrar / Guardar Tema comercial", fg_color=t["btn_exito"], hover_color=t["btn_exito_hover"], command=self.guardar_tema)
        ...
        # PANEL DERECHO DE SELECCIÓN DE ESQUEMAS
    ...
    def actualizar_muestra_desde_texto(self, clave_ref):
        try:
            ...
            elif clave_ref == "p_bg_sel": val_txt = self.ent_p_bg_sel.get()     # v32
            elif clave_ref == "buscar_txt": val_txt = self.ent_buscar_txt.get()   # v32
            elif clave_ref == "buscar_bg": val_txt = self.ent_buscar_bg.get()     # v32
            elif clave_ref == "buscar_border": val_txt = self.ent_buscar_border.get() # v32
    ...
    def al_seleccionar_registro(self, event):
        ...
        if r:
            ...
            # v32: Rellenar nuevos campos indexados
    ...
    def limpiar_formulario(self):
        ...
        self.ent_p_bg_sel.delete(0, tk.END) # v32
        self.ent_buscar_txt.delete(0, tk.END) # v32
        self.ent_buscar_bg.delete(0, tk.END)  # v32
        self.ent_buscar_border.delete(0, tk.END) # v32
...
def seleccionar_prompt(id_seleccionado, frame_asociado, frame_contenedor):
    ...
    # v32: Usar el color de fondo dinámico guardado en el Grupo 3 para la tarjeta activa
...
def filtrar_prompts_ui(event=None):
    ...
    for reg in resultados:
        ...
        # v32: Renderizado reactivo con fondos personalizados
...
def cambiar_tema_aplicacion(nombre_tema):
    if nombre_tema in PALETAS:
        ...
        if root:
            ...
            # v32: Refrescar el estilo del buscador en la ventana principal
            if txt_buscar:
                txt_buscar.configure(
                    text_color=t.get("buscar_text_color", "#E2E8F0"),
                    fg_color=t.get("buscar_fg_color", "#111827"),
                    border_color=t.get("buscar_border_color", "#10B981")
                )
        filtrar_prompts_ui()
...
def iniciar_aplicacion():
    ...
    root = ctk.CTk()
    root.title("PromptVault Premium v32")
    ...
    # v32: Campo de búsqueda con carga de estilos dinámicos desde DB
    txt_buscar = ctk.CTkEntry(
        frame_izq, 
        placeholder_text="Buscar prompt...",
        text_color=t.get("buscar_text_color", "#E2E8F0"),
        fg_color=t.get("buscar_fg_color", "#111827"),
        border_color=t.get("buscar_border_color", "#10B981")
    )

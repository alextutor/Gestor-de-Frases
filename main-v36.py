...
def gestionar_navegacion_teclado(event):
    ...
    # Si el evento viene de la raíz pero el foco está en el buscador, ignoramos 
    # para dejar que el binding del buscador controle el evento único.
    if event.widget == root and root.focus_get() == txt_buscar:
        return

    if event.keysym == "Down":
        INDICE_SELECCION_TECLADO = (INDICE_SELECCION_TECLADO + 1) % cantidad_tarjetas
...
def ejecutar_enter_teclado(event):
    ...
    # Intercepción de seguridad contra disparos duplicados
    if event.widget == root and root.focus_get() == txt_buscar:
        return
...
def iniciar_aplicacion():
    ...
    # Binds directos al buscador con intercepción de eventos
    ...
    # Binds Globales para navegación cuando el buscador no tiene el foco

...
# RE ESTRUCTURA NATIVA PARA WINDOWS 10 (ÁREA DE TRABAJO REAL)
class RECT(ctypes.Structure):
    _fields_ = [("left", ctypes.c_long), ("top", ctypes.c_long), 
                ("right", ctypes.c_long), ("bottom", ctypes.c_long)]

def centrar_ventana(ventana, padre=None):
    # Usamos update() en lugar de update_idletasks() para asegurar el render de dimensiones reales
    ventana.update()
    ...
    if padre:
        ...
        # SPI_GETWORKAREA = 48 -> Excluye barras de tareas arriba, abajo, izquierda o derecha
        rect = RECT()
        ...
        x = rect.left + (work_width // 2) - (width // 2)
        y = rect.top + (work_height // 2) - (height // 2)
...
# ==========================================
# CRUD DE TEMAS COMERCIALES (INTERFAZ CORREGIDA)
# ==========================================
class VentanaCrudTemas(ctk.CTkToplevel):
    def __init__(self, master, callback_recargar_menu, *args, **kwargs):
        ...
        # CORREGIDO: "yard=10" cambiado por el estándar "pady=10"
        ...
        def crear_input_color(parent, label_text, placeholder, clave_ref):
            ...
            # Recuadro visualizador que representa el color escogido
        ...
        # Botón salir en el extremo derecho inferior del panel izquierdo
        frame_salir_panel = ctk.CTkFrame(frame_form, fg_color="transparent")
        frame_salir_panel.pack(fill="x", padx=10, pady=(0, 10))
        self.btn_salir = ctk.CTkButton(frame_salir_panel, text="Salir", fg_color=t["btn_neutral"], command=self.destroy)
    ...
    def actualizar_muestra_desde_texto(self, clave_ref):
        try:
            ...
            if val_txt.startswith("#") and (len(val_txt) == 4 or len(val_txt) == 7):
                self.visualizadores[clave_ref].configure(fg_color=val_txt)
        except Exception: pass
...
def iniciar_aplicacion():
    ...
    root = ctk.CTk()
    root.title("PromptVault Premium v28")

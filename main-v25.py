...
PROMPTS_CACHE = []         
PALETAS = {}  # Se cargará dinámicamente desde la Base de Datos
...
def centrar_ventana(ventana, padre=None):
    ...
    if padre:
        # Centrado relativo al contenedor padre
        ...
        # Centrado absoluto en la pantalla principal
...
def inicializar_db_y_config():
    ...
    try:
        ...
        # NUEVA: Tabla de Temas Comerciales Dinámicos
        cursor.execute('''
        ...
        # Inserción de valores base de fábrica
        ...
        # Inserción de Temas Premium por defecto
        temas_defecto = [
            ('Slate Corporate', 'dark', 'blue'),
            ('Emerald Obsidian', 'dark', 'green'),
            ('Steel Amber', 'dark', 'dark-blue'),
            ('Classic Light', 'light', 'blue')
        ]
        for nom, mod, col in temas_defecto:
            cursor.execute("INSERT OR IGNORE INTO temas_comerciales (nombre, modo, color) VALUES (?, ?, ?)", (nom, mod, col))
...
def cargar_configuracion_global():
    ...
    try:
        ...
        # 1. Cargar Temas desde la Base de Datos a la memoria
        cursor.execute("SELECT nombre, modo, color FROM temas_comerciales")
        PALETAS = {}
        for nom, mod, col in cursor.fetchall():
            PALETAS[nom] = {"mode": mod, "color": col}
            
        # 2. Cargar variables globales
...
class VentanaPapeleraReciclaje(ctk.CTkToplevel):
    def __init__(self, master, *args, **kwargs):
        ...
        ctk.CTkLabel(self, text="Historial de Prompts Eliminados", font=("Arial", 14, "bold")).pack(pady=10)
        ...
        frame_ctrl.pack(fill="x", pady=10, padx=15)
        
        btn_salir = ctk.CTkButton(frame_ctrl, text="Salir", fg_color="#444444", hover_color="#333333", command=self.destroy)
        btn_salir.pack(side="right", padx=5)
        
        self.cargar_eliminados()

    def cargar_eliminados(self):
        for widget in self.scroll_papelera.winfo_children():
            widget.destroy()
        ...
        for r_id, r_titulo, r_fecha in registros:
            ...
            lbl_info = ctk.CTkLabel(fila, text=info_text, anchor="w", justify="left")
            lbl_info.pack(side="left", fill="x", expand=True)
            
            btn_restaurar = ctk.CTkButton(fila, text="↺ Restaurar", width=95, height=28, fg_color="#10B981", hover_color="#059669",
...
class VentanaConfigurarPurga(ctk.CTkToplevel):
    def __init__(self, master, *args, **kwargs):
        ...
        ctk.CTkLabel(self, text="Plazo de Purga Automática", font=("Arial", 13, "bold")).pack(pady=(15, 5))
        ctk.CTkLabel(self, text="Días de retención antes del borrado definitivo:", font=("Arial", 11)).pack(pady=2)
        ...
        ctk.CTkButton(self, text="Guardar Plazo", command=self.guardar_plazo).pack(pady=5)
...
class VentanaControlPreventivo(ctk.CTkToplevel):
    def __init__(self, master, *args, **kwargs):
        ...
        # Botonera Aceptar y Salir
        frame_btns = ctk.CTkFrame(self, fg_color="transparent")
        frame_btns.pack(fill="x", side="bottom", pady=15, padx=25)
        
        btn_salir = ctk.CTkButton(frame_btns, text="Salir", fg_color="#444444", hover_color="#333333", width=100, command=self.destroy)
        btn_salir.pack(side="right", padx=(10, 0))
        
        btn_aceptar = ctk.CTkButton(frame_btns, text="Aceptar", width=100, command=self.confirmar_guardado)
...
# ==========================================
# CRUD DE TEMAS COMERCIALES (OPCIÓN A)
# ==========================================
class VentanaCrudTemas(ctk.CTkToplevel):
    def __init__(self, master, callback_recargar_menu, *args, **kwargs):
        ...
        self.title("Administración de Temas Comerciales")
        ...
        # Layout Split: Izquierda Formulario, Derecha Tabla
        ...
        ctk.CTkLabel(frame_form, text="Datos del Tema", font=("Arial", 12, "bold")).pack(pady=10)
        
        ctk.CTkLabel(frame_form, text="Nombre comercial:").pack(anchor="w", padx=15)
        self.ent_nombre = ctk.CTkEntry(frame_form)
        self.ent_nombre.pack(fill="x", padx=15, pady=5)
        
        ctk.CTkLabel(frame_form, text="Modo Visual:").pack(anchor="w", padx=15)
        self.cmb_modo = ctk.CTkComboBox(frame_form, values=["dark", "light"], state="readonly")
        self.cmb_modo.pack(fill="x", padx=15, pady=5)
        self.cmb_modo.set("dark")
        
        ctk.CTkLabel(frame_form, text="Color Base (Theme):").pack(anchor="w", padx=15)
        self.cmb_color = ctk.CTkComboBox(frame_form, values=["blue", "green", "dark-blue"], state="readonly")
        self.cmb_color.pack(fill="x", padx=15, pady=5)
        self.cmb_color.set("blue")
        
        # Botonera CRUD
        self.btn_guardar = ctk.CTkButton(frame_form, text="Registrar Tema", command=self.guardar_tema)
        self.btn_guardar.pack(fill="x", padx=15, pady=10)
        
        self.btn_limpiar = ctk.CTkButton(frame_form, text="Limpiar / Nuevo", fg_color="#555555", hover_color="#444444", command=self.limpiar_formulario)
        self.btn_limpiar.pack(fill="x", padx=15, pady=2)
        
        self.btn_eliminar = ctk.CTkButton(frame_form, text="Eliminar Selección", fg_color="#D32F2F", hover_color="#B71C1C", command=self.eliminar_tema)
        self.btn_eliminar.pack(fill="x", padx=15, pady=12)
        self.btn_eliminar.configure(state="disabled")
        
        # --- PANEL DERECHO: LISTA / TREEVIEW ---
        frame_lista = ctk.CTkFrame(self)
        frame_lista.grid(row=0, column=1, sticky="nsew", padx=10, pady=10)
        
        ctk.CTkLabel(frame_lista, text="Temas Disponibles en el Sistema", font=("Arial", 12, "bold")).pack(pady=10)
        
        # Tabla Estilo Nativo Tkinter dentro de CustomTkinter
        estilo = ttk.Style()
        estilo.theme_use("clam")
        estilo.configure("Treeview", background="#242424", foreground="white", fieldbackground="#242424", rowheight=24)
        estilo.map("Treeview", background=[("selected", "#1F6AA5")])
        
        self.tabla = ttk.Treeview(frame_lista, columns=("Nombre", "Modo", "Color"), show="headings")
        self.tabla.heading("Nombre", text="Nombre")
        self.tabla.heading("Modo", text="Modo")
        self.tabla.heading("Color", text="Color Base")
        self.tabla.column("Nombre", width=140)
        self.tabla.column("Modo", width=60)
        self.tabla.column("Color", width=80)
    ...
    def actualizar_tabla_ui(self):
        ...
        try:
            conn = sqlite3.connect("prompts.sqlite")
            cursor = conn.cursor()
            cursor.execute("SELECT id, nombre, modo, color FROM temas_comerciales ORDER BY id ASC")
            for fila in cursor.fetchall():
    ...
    def al_seleccionar_registro(self, event):
        ...
        valores = self.tabla.item(self.id_tema_seleccionado, "values")
        
        self.ent_nombre.delete(0, tk.END)
        self.ent_nombre.insert(0, valores[0])
        self.cmb_modo.set(valores[1])
        self.cmb_color.set(valores[2])
        
        self.btn_guardar.configure(text="Actualizar Tema")
    ...
    def guardar_tema(self):
        ...
        if self.id_tema_seleccionado:
            cursor.execute("UPDATE temas_comerciales SET nombre=?, modo=?, color=? WHERE id=?", (nombre, modo, color, self.id_tema_seleccionado))
        else:
            cursor.execute("INSERT OR IGNORE INTO temas_comerciales (nombre, modo, color) VALUES (?, ?, ?)", (nombre, modo, color))
    ...
    def eliminar_tema(self):
        ...
        # Control preventivo: No dejar borrar el set base si está activo
        global TEMA_ACTUAL
        if nombre_tema == TEMA_ACTUAL or nombre_tema == "Slate Corporate":
            return
...
def iniciar_aplicacion():
    global root, frame_lista_global, txt_buscar, submenu_temas
    ...
    root = ctk.CTk()
    root.title("PromptVault v25")
    root.geometry("450x540")
    
    # Arreglo 1: Centrado absoluto al iniciar la ventana principal
